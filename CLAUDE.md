...
- Never hand-edit or manually run `format-lang` to regenerate `src/scripts/out/langPack.strings` — it is auto-generated from `lang.ts`/`langSign.ts` by the Vite `tweb-lang` plugin (`scripts/vite-plugin-lang.js`) on dev-server start, on every `lang.ts` change, and on build. Edit the lang `.ts` source only.
...
- Do not run preview or use chrome devtools MCP unprompted. Always ask the user for permission.
- Do not overuse comments. Comments are for explaining why, not for explaining what.
