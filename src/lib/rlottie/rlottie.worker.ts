...
export class RLottieItem {
  ...
  private canvases: OffscreenCanvas[] | undefined;
  private contexts: OffscreenCanvasRenderingContext2D[];
  private cacheName: string | undefined;
  private cacheEntry: WorkerFramesCacheEntry | undefined;
  private cachingDelta: number;
  private color: string | undefined;
  private compositorDelivery: boolean | undefined;
  private stagedFrame: ImageBitmap | undefined;
  ...
  public setColor(color: string | undefined, reTint: boolean) {
    this.color = color;
...
const withItem = <T>(reqId: number, callback: (item: RLottieItem) => T): T => {
  ...
  return undefined!; // item already destroyed mid-flight - the RPC layer tolerates a void ack
};
