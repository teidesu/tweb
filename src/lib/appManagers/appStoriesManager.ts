...
import { SERVICE_PEER_ID, TEST_NO_STORIES } from '@/lib/appManagers/constants';
...
type StoriesPeerCache = {
  ...
  getStoriesPromise?: CancellablePromise<void> | Promise<void>,
...
export default class AppStoriesManager extends AppManager {
  ...
  public getPeerStoriesCache(peerId: PeerId, create = true): StoriesPeerCache {
    const newCache: StoriesPeerCache = create ? {
      peerId,
      stories: [],
      pinnedStories: [],
      archiveStories: [],
  ...
  public getAlbumStoriesCacheSnapshot(peerId: PeerId, albumId: number) {
    ...
    const stories = album ?
      album.ids.map((storyId) => cache.storiesMap.get(storyId)).filter(isTruthy) :
      [];
    return {
      stories,
  ...
  public getStoriesById(peerId: PeerId, ids: StoryItem['id'][], overwrite?: boolean): MaybePromise<StoryItem.storyItem[]> {
    ...
    return hasPromise ? (Promise.all(arr) as MaybePromise<StoryItem.storyItem[]>) : arr as StoryItem.storyItem[];
  }
