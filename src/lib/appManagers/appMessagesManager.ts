...
import { FOLDER_ID_ALL, FOLDER_ID_ARCHIVE, GENERAL_TOPIC_ID, HIDDEN_PEER_ID, MESSAGES_ALBUM_MAX_SIZE, MUTE_UNTIL, NULL_PEER_ID, REAL_FOLDERS, REAL_FOLDER_ID, REPLIES_HIDDEN_CHANNEL_ID, REPLIES_PEER_ID, SERVICE_PEER_ID, TEST_NO_SAVED, THUMB_TYPE_FULL, TOPIC_COLORS } from '@/lib/appManagers/constants';
...
import { isDialog, isSavedDialog, isForumTopic, isMonoforumDialog } from '@/lib/appManagers/utils/dialogs/isDialog';
import getDialogKey from '@/lib/appManagers/utils/dialogs/getDialogKey';
import getHistoryStorageKey, { getSearchStorageFilterKey } from '@/lib/appManagers/utils/messages/getHistoryStorageKey';
...
import createHistoryStorage, { createHistoryStorageSearchSlicedArray } from '@/lib/appManagers/utils/messages/createHistoryStorage';
...
export class AppMessagesManager extends AppManager {
  ...
  private fetchSingleMessagesPromise: Promise<void> | undefined;
  ...
  private newDialogsHandlePromise: Promise<any> | undefined;
  ...
  private notificationsHandlePromise: number | undefined;
  ...
  private reloadConversationsPromise: Promise<void> | null;
  ...
  public makeDocumentAndMetaForSendingFile(args: MakeDocumentAndMetaForSendingFileArgs) {
    ...
    if (
      ...
      document = (this.appDocsManager.saveDoc(document) as Document.document);
    }
  ...
  public async sendGrouped(options: MessageSendingParams & {
    ...
    const _results = options.sendFileDetails.map(async(details, idx) => {
      const o: Parameters<AppMessagesManager['sendFile']>[0] = {
        ...
        groupedMessage: ((isSingleMessageForAlbum && firstMessage) as Message.message | undefined),
        groupId,
        stars: options.stars,
        processAfter,
        ...details,
    ...
    }) as Promise<InputSingleMedia.inputSingleMedia>[]);
  ...
  public generateOutgoingMessage(
    ...
    const message: Message.message = {
      ...
      views: ((isBroadcast && 1) as number | undefined),
      pending: true,
      effect: options.effect,
  ...
  public isAnonymousSending(peerId: PeerId): boolean {
    ...
    return (chat as Chat.channel).admin_rights?.pFlags?.anonymous as boolean;
  }
  ...
  public async forwardMessagesInner(options: MessageForwardParams) {
    ...
    const send = () => this.apiManager.invokeApiAfter('messages.forwardMessages', {
      ...
      random_id: (newMessages.map((message) => message.random_id) as (string | number)[]),
  ...
  public generateEmptyMessage(mid: number): Message.message | Message.messageService | undefined {
    return undefined;
    // return {
    //   _: 'messageEmpty',
  ...
  private historyStorageAsTransferable(historyStorage: HistoryStorage) {
    ...
    const ret: any = {
      ...historyStorageCopy,
      history: undefined as unknown as HistoryStorage,
  ...
  public mergeHistoryResult(options: RequestHistoryOptions & {
    ...
    if (!count) {
      ...
      slice = slicedArray.insertSlice((mids as number[]))!;
      hadSlice = !!slice;
      slice ||= slicedArray.slice;
    }
  ...
  private async fillHistoryStorage(options: RequestHistoryOptions & {
    ...
    if (options.threadId) {
      if (isTopEnd && options.historyType === HistoryType.Thread) {
        ...
        historyStorage.history!.insertSlice((addSlice as number[]));
  ...
  public requestHistory({
    ...
    offsetPeerId ??= (offsetMessage as Message.message)?.peerId;
  ...
  private readMetricsFlushTimeout: number | undefined;
