...
type CompositorRenderer = {canvas: OffscreenCanvas, context: OffscreenCanvasRenderingContext2D, dpr: number, color: string | undefined, fadeEnabled: boolean, groups: Map<DocId, CompositorGroup>, dirty: boolean, suspended: boolean};
...
compositorMessagePort.addMultipleEventsListeners({
  ...
  attachSticker: ({ reqId, canvases, color }) => {
    ...
    const sticker: StickerRenderer = { surfaces, color: color! };
