...
export const Result = (props: {
  ...
  const [translation] = createResource(() => props.language, (lang) => {
    const msgParams: Parameters<typeof rootScope.managers.appTranslationsManager.translateText>[0] = props.message ?
      { peerId: props.message.peerId!, mid: props.message.mid!, lang } :
      { text: props.textWithEntities!, lang };
    return rootScope.managers.appTranslationsManager.translateText(msgParams);
  });
          ...
          <Switch>
            <Match when={translation.state === 'ready' && translation()} keyed>
              {(text) => (
                ...
                >
                  <div
                    class={clsx(previewStyles.richTextScrollableContent, 'spoilers-container')}
                    dir='auto'
