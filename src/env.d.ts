...
interface ImportMetaEnv {
  ...
  // injected via `define` by scripts/preview/vite.config.ts; absent in every other build
  readonly VITE_PREVIEW?: boolean;
  // injected via `define` by scripts/preview/vite.config.ts when scripts/preview/start.sh is
